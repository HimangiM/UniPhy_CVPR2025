...
class MPMDataset(Dataset):
    ...
    def __init__(self, root: Union[str, Path], device: torch.device) -> None:
        ...
        with torch.no_grad():
            ...
            # self.xs = torch.load(self.root / 'GtX.pt', map_location=device)
            # self.vs = torch.load(self.root / 'GtV.pt', map_location=device)
            # self.Cs = torch.load(self.root / 'GtC.pt', map_location=device)
            # self.Fs = torch.load(self.root / 'GtF.pt', map_location=device)
            # self.stresss = torch.load(self.root / 'GtStress.pt', map_location=device)
