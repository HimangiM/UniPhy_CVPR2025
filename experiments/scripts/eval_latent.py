...
def main(cfg: DictConfig):
    ...
    elasticity = ComposeMaterialLatent(elasticities, sections)
    elasticity.to(torch_device)
    elasticity.requires_grad_(requires_grad)
    elasticity.eval()

    plasticity = ComposeMaterialLatent(plasticities, sections)
    plasticity.to(torch_device)
    ...
    for step in trange(1, cfg.sim.num_steps + 1):
        if cfg.dataset:
            Fe = F.clone()
        stress = elasticity(F, trajectory_latent)
        state.from_torch(stress=stress)
        x, v, C, F = sim(statics, state)
        if cfg.dataset:
            Fp = F.clone()
        F = plasticity(F, trajectory_latent)
