...
def falling_under_gravity(random_mu, random_lam):
    ...
    for idx in range(len(random_mu)):
        cur_obj = objects[idx]
        os.system(f"python dataset_generate.py objects=[{cur_obj}] objects.{cur_obj}.geometry.num_particles=500 objects/{cur_obj}/material=elastic visualization_cfg.num_frames=10 "
                  f"objects.{cur_obj}.geometry.pos_y=0.2 train_cfg.cuda_chunk_size=4096 train_cfg.particles_ti_root=1024 "
                  f"train_cfg.local_dir={local_path} train_cfg.save_dir='dataset/elastic_diverse/elastic_{cur_obj}_mu{random_mu[idx]}_lam{random_lam[idx]}' "
...
def horizontal_left(random_mu, random_lam):
    ...
    for idx in range(len(random_mu)):
        ...
        os.system(f"python dataset_generate.py objects=[{cur_obj}] objects.{cur_obj}.geometry.num_particles=500 objects/{cur_obj}/material=elastic visualization_cfg.num_frames=10 "
                  ...
                  f"train_cfg.local_dir={local_path} train_cfg.save_dir='dataset/elastic_diverse/elastic_{cur_obj}_horizontaleft_mu{random_mu[idx]}_lam{random_lam[idx]}' "
                  f"objects.{cur_obj}.material.mu={random_mu[idx]} objects.{cur_obj}.material.lam={random_lam[idx]}")
    return
...
def horizontal_right(random_mu, random_lam):
    ...
    for idx in range(len(random_mu)):
        ...
        os.system(f"python dataset_generate.py objects=[{cur_obj}] objects.{cur_obj}.geometry.num_particles=500 objects/{cur_obj}/material=elastic visualization_cfg.num_frames=10 "
                  f"objects.{cur_obj}.geometry.pos_y=0.13 objects.{cur_obj}.geometry.pos_x=0 objects.{cur_obj}.material.velocity='[-2., 0., 0.]' "
                  f"train_cfg.cuda_chunk_size=4096 train_cfg.particles_ti_root=1024 "
                  f"train_cfg.local_dir={local_path} train_cfg.save_dir='dataset/elastic_diverse/elastic_{cur_obj}_horizontalright_mu{random_mu[idx]}_lam{random_lam[idx]}' "
                  f"objects.{cur_obj}.material.mu={random_mu[idx]} objects.{cur_obj}.material.lam={random_lam[idx]}")
    return

def diagonal_left(random_mu, random_lam):
    object = ["blobby", "torus", "blobby", "torus", "toy", "knurling", "pawn", "ellipsoid", "blobby", "knurling"] * 5
    for idx in range(len(random_mu)):
        cur_obj = object[idx]
        os.system(f"python dataset_generate.py objects=[{cur_obj}] objects.{cur_obj}.geometry.num_particles=500 objects/{cur_obj}/material=elastic visualization_cfg.num_frames=10 "
                  f"objects.{cur_obj}.geometry.pos_y=0.2 objects.{cur_obj}.geometry.pos_x=0 objects.{cur_obj}.material.velocity='[2., 0., 0.]' "
                  f"train_cfg.cuda_chunk_size=4096 train_cfg.particles_ti_root=1024 "
                  f"train_cfg.local_dir={local_path} train_cfg.save_dir='dataset/elastic_diverse/elastic_{cur_obj}_diagonalleft_mu{random_mu[idx]}_lam{random_lam[idx]}' "
                  f"objects.{cur_obj}.material.mu={random_mu[idx]} objects.{cur_obj}.material.lam={random_lam[idx]}")
    return

if __name__ == '__main__':
    min_range_mu, max_range_mu = 350.0, 2595196.0
    min_range_lam, max_range_lam = 500.0, 2580120.0
    num_samples = 1
    random_mu = []
