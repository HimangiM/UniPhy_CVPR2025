...
@ti.data_oriented
class MPMSimulatorLearnableGrad:
    ...
    def __init__(self, dtype, dt, frame_dt, particle_layout, dx, inv_dx, n_particles, gravity=[0, -9.8, 0], material=elasticity, cuda_chunk_size=400, embed_dim=1024, **kargs):
        ...
        self.stress = ti.Matrix.field(dim, dim, dtype=self.dtype, needs_grad=True)
        ...
        # [E 08/26/24 16:33:02.313 208131] [cuda_driver.h:operator()@92] CUDA Error CUDA_ERROR_ASSERT: device-side assert triggered while calling mem_free (cuMemFree_v2)
        # self.traj_latent_ti = ti.Vector.field(embed_dim, dtype=self.dtype, needs_grad=True)
        ...
        self.cohesion = ti.field(dtype=self.dtype, needs_grad=True)
        ...
        # RuntimeError: [struct_llvm.cpp:run@266] Assertion failure: (int)snodes.size() <= taichi_max_num_snodes; taichi_max_num_snodes = 1024; embed_dim=~240
        # self.traj_latent_ti = ti.Vector.field(embed_dim, dtype=self.dtype, needs_grad=True)
        self.traj_latent_ti = ti.field(dtype=self.dtype, needs_grad=True, shape=(embed_dim))
        ...
        grid = self.grid = ti.root.pointer(ti.ijk, grid_size // grid_block_size) # 32 x 32 x 32
        block = self.block = grid.pointer(ti.ijk, grid_block_size // leaf_block_size) # 1024 x 1024 x 1024
        ...
        def block_component(c):
            block.dense(ti.ijk, leaf_block_size).place(c, c.grad, offset=offset)  # 4 x 4 x 4

        block_component(self.grid_m)     # grid_m = 4096 x 4096 x 4096
        block_component(self.grid_v_in)  # grid_v_in = 4096 x 4096 x 4096
        block_component(self.grid_v_out) # grid_v_out = 4096 x 4096 x 4096
    ...
    def clear_grads(self):
        ...
        # self.traj_latent_ti.grad.fill(0)
    ...
    def svd(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            self.U[p, s+1], self.sig[p, s+1], self.V[p, s+1] = ti.svd(self.F_tmp[p, s+1].cast(ti.f64))

    @ti.kernel
    def svd_grad(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            self.F_tmp.grad[p, s+1] = self.F_tmp.grad[p, s+1] + (self.backward_svd(self.U.grad[p, s+1].cast(ti.f64), self.sig.grad[p, s+1].cast(ti.f64), self.V.grad[p, s+1].cast(ti.f64), self.U[p, s+1].cast(ti.f64), self.sig[p, s+1].cast(ti.f64), self.V[p, s+1].cast(ti.f64)))

    @ti.kernel
    def project_F(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            sig = ti.Vector([ti.max(self.sig[p, s+1][0,0], 0.05), ti.max(self.sig[p, s+1][1,1], 0.05), ti.max(self.sig[p, s+1][2,2], 0.05)])
            ...
            if self.material[p] == self.viscous_fluid:
                ...
                self.sig_out[p, s+1] = sig_out
                self.F[p, s+1] = self.U[p, s+1] @ self.make_matrix_from_diag(sig_out) @ self.V[p, s+1].transpose()
    ...
    def p2g(self, s: ti.i32):
        ...
        for p in range(self.n_particles[None]):
            ...
            if self.material[p] == self.elasticity:
                ...
                log_sig = ti.log(self.sig_out[p, s+1])
                tau = 2 * self.mu[p] * log_sig + self.lam[p] * log_sig.sum()
                stress = self.U[p, s+1] @ self.make_matrix_from_diag(tau) @ self.U[p, s+1].transpose()
    ...
    def check_cfl(self, s: ti.i32):
        for p in range(self.n_particles[None]):
            if ti.math.isnan(self.v[p, s]).any():
                ...
                print ("Cfl: nan in v", p, s)
            if ti.abs(self.v[p, s]).max() * self.dt[None] > self.dx[None]:
                self.cfl_satisfy[None] = 0
                print ("Cfl: v*dt > x", p, s, ti.abs(self.v[p, s]).max(), self.dt[None], self.dx[None])
